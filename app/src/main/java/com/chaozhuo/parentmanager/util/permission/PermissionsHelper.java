package com.chaozhuo.parentmanager.util.permission;
...
public class PermissionsHelper {
    ...
    private void requestPermission(final String[] permissions) {
        PermissionsHelper.init(null).requestEachPermissions(permissions, new IPermissionListenerWrap.IEachPermissionListener() {
            @Override
    ...
    private void show(Permission permission) {
        if (permission.granted) {
            Logger.v("授予权限 ：" + permission.name);
        } else {
            if (permission.shouldShowRequestPermissionRationale) {
                Logger.d("没有勾选不再提醒，拒绝权限 ：" + permission.name);
            } else {
                Logger.e("勾选不再提醒，拒绝权限 ：" + permission.name);
            }
        }
    }
