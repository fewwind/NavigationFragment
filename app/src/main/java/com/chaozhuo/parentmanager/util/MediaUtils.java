...
import com.chaozhuo.parentmanager.bean.MediaData;
...
public class MediaUtils {
            ...
            public void run() {
                ...
                if (mCursor != null) {
                    while (mCursor.moveToNext()) {
                        ...
                        //判断文件是否存在，存在才去加入
                        boolean b = FileUtils.fileIsExists(filepath);
                        if (b) {
    ...
    private static void getUriColumns(Uri uri) {
        Cursor cursor = App.app.getContentResolver().query(uri, null, null, null, null);
        cursor.moveToFirst();
