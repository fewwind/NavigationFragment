...
public class HotFixUtil {
    ...
    public void startFix() throws IllegalAccessException, NoSuchFieldException, ClassNotFoundException {
        ...
        File sourceFile = App.app.getExternalFilesDir(SOURCE_DIR);
        if (!sourceFile.exists()) {
            return;
        }
        // 默认 dex优化存放目录  /data/data/rocketly.hotfixdemo/app_odex
        File optFile = App.app.getDir(OPTIMIZE_DIR, Context.MODE_PRIVATE);
        if (!optFile.exists()) {
            optFile.mkdir();
        }
        ...
        ClassLoader pathClassLoader = App.app.getClassLoader();//拿到系统默认的PathClassLoader加载器
        DexClassLoader dexClassLoader = new DexClassLoader(dexPath, optPath, null, App.app.getClassLoader());
